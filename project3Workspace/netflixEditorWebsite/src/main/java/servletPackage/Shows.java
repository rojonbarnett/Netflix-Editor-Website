...
public class Shows {
	...
	public Shows() {
		setShowWeeks(new ArrayList<ShowWeek>());
		fileName = null;
	...
	public ShowWeek find(String t, String w) {
		int index = getShowWeeks().indexOf(new ShowWeek(t,w));
		if(index == -1) {
			return null;
		}
		return getShowWeeks().get(index);
	}

	//get random ShowWeek 
	public ShowWeek randomShow(){
		while (true) {
			int index = generator.nextInt(0,getShowWeeks().size());
			ShowWeek sw = getShowWeeks().get(index);
			if (!sw.isPurged())
			return sw;
		}
	}
	...
	public void purgeShow(String t) {
		for (ShowWeek s : getShowWeeks()) {
			if(s.getShowTitle().equals(t)) {
				s.setShowTitle("*"+t);
			}
	...
	public void removeShow(String showTitle)
	{
		...
		for (ShowWeek s : getShowWeeks())
		{
			if(s.getShowTitle().equals(showTitle)) {
				getShowWeeks().removeAll(Collections.singleton(s));
			}
		}
	}
	...
	public ArrayList<ShowWeek> getShowWeeks() {
		return showWeeks;
	}

	public void setShowWeeks(ArrayList<ShowWeek> showWeeks) {
